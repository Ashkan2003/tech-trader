...
interface Props {
  title: string;
  color: "primary" | "secondary" | "ternery";
}
...
const Badge = ({ title, color }: Props) => {
  ...
  if (color == "ternery")
    return (
      <span className="inline-flex font-sans items-center justify-center rounded-md   py-[0.5px] px-1 text-xs font-medium text-green-500 dark:text-green-500">
        {title}
      </span>
    );
};
